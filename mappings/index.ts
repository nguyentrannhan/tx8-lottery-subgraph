...
export function handleLotteryOpen(event: LotteryOpen): void {
  ...
  lottery.ticketPrice = toBigDecimal(event.params.priceTicketInTX8);
...
export function handleTicketsPurchase(event: TicketsPurchase): void {
  ...
  if (user === null) {
    ...
    user.totalTX8 = ZERO_BD;
    user.block = event.block.number;
    user.timestamp = event.block.timestamp;
    user.save();
  }
  user.totalTickets = user.totalTickets.plus(event.params.numberTickets);
  user.totalTX8 = user.totalTX8.plus(event.params.numberTickets.toBigDecimal().times(lottery.ticketPrice));
  user.save();
...
export function handleTicketsClaim(event: TicketsClaim): void {
  ...
  if (user !== null) {
    user.totalTX8 = user.totalTX8.plus(toBigDecimal(event.params.amount));
    user.save();
  }
